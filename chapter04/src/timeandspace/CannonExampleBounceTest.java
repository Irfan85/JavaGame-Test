...
public class CannonExampleBounceTest extends JFrame implements Runnable {
	...
	private Vector2f bullet; // Geometry vector (A single point for our case) for the bullet to modify its position
...
//		canvas.addMouseWheelListener(mouseInput);
	...
	private void processInput(double timeDelta) {
		...
		if(keyboardInput.keyDownOnce(KeyEvent.VK_SPACE)) {
			...
			// It doesn't matter the vector is initialized at (0, 0) since the transformation matrix will manipulate it and put it in the correct position
	...
	private void updateObjects(double timeDelta) {
		...
		if(bullet != null) {			
			// Bounce
			if (bullet.y >= -2.6f && bullet.y <= -2.3f) {
//				System.out.println("OK. Velocity : " + velocity.y);
				velocity.y = (float) Math.abs(0.7 * velocity.y);
				velocity.x = (float) Math.abs(0.7 * velocity.x);
			} else {
				// Don't bounce
	...
	public static void main(String[] args) {
		final CannonExampleBounceTest app = new CannonExampleBounceTest();
